...
{
  	// Reading Data
	string filename_throughput = "../../Throughput/throughput6_slowdrive.txt";
	string filename_trace = "mobility_1534394649.txt";
	string filename_write = "Throughput_Mobility_6.txt";
	...
	if (rstr1.is_open() && rstr2.is_open())
	{
		...
		while( !rstr1.eof()| !rstr2.eof())
		{
			...
			double throughput = stod(strOne.substr(2, 5));
			
			if(throughput > 100.0)
				throughput = throughput/1000;
			//cout << time << " " << throughput << endl;
			...
			wstr << time << "," << latitude << "," << longitude << "," << throughput << endl;
			wstr.close();
			cout << time << "," << latitude << "," << longitude << "," << throughput << endl;
